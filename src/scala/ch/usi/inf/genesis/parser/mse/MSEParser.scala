...
object MSEParser extends RegexParsers {
          ...
          obj match {
            case Element(name, _, _) =>
              name match {
                ...
                case FAMIX.PACKAGE =>
                  this.modelObject.addProperty(FAMIX.PACKAGES_PROP, obj.modelObject)
                case _ =>
              }
            case _ =>
          }
  ...
  case class Element(val name: String, val modelName: String, val id: Option[Int]) extends MSEEntity {
    ...
    override def resolve(pool: Map[Int, MSEEntity]): Option[ModelObject] = {
      ...
      name match {
        ...
        case FAMIX.PACKAGE => obj = Some(new PackageEntity())
  ...
  def serial = open ~> id ~> natural <~ close ^^ {
    case naturalValue => naturalValue.toInt
  }
  ...
  def boolean = (booleanTrue | booleanFalse) ^^ {
    case booleanValue => new Value(new BooleanValue(booleanValue.toBoolean))
  }
  ...
  def nameReference = open ~> ref ~> elementName <~ close ^^ {
    case (model, name) => new StringReference(model + "." + name)
  }
  ...
  def nameIdentifier = ((letter | digit) *) ^^ {
    case xs => xs.foldLeft("")((element, rest) => element + rest)
  }
  ...
  def simpleName = letter ~ nameIdentifier <~ (space *) ^^ {
    case l ~ nid => l + nid
  }

  def natural = (digit +) ^^ {
    case xs => xs.foldLeft("")((v, rest) => v + rest)
  }

  def suffix = "." ~> natural ^^ {
    case naturalNumber => "." + naturalNumber
  }
  ...
  def string = "'[^']*'".r ^^ {
    str => new Value(new StringValue(str.substring(1, str.length() - 1)))
  }
